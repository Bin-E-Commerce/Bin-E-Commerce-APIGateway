...
export class SellerOrderProxyController {
    ...
    async proxyReturnList(
        @Req() request: Request,
        @Res() response: Response,
    ): Promise<void> {
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/seller/orders/returns`,
            request,
        );
        response.status(status).json(data);
    }
    ...
    async approveReturn(
        ...
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/seller/orders/returns/${returnId}/approve`,
            request,
        );
        response.status(status).json(data);
    }
    ...
    async rejectReturn(
        ...
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/seller/orders/returns/${returnId}/reject`,
            request,
        );
        response.status(status).json(data);
    }
    ...
    async inspectReturn(
        ...
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/seller/orders/returns/${returnId}/inspection`,
            request,
        );
        response.status(status).json(data);
    }
