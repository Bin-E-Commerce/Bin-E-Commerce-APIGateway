...
export class OrderProxyController {
    ...
    async proxyOrderQuote(
        @Req() request: Request,
        @Res() response: Response,
    ): Promise<void> {
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/orders/quote`,
            request,
        );
    ...
    @RequirePermissions(Permission.ORDER_CREATE)
    async proxyShippingAddressResolve(
        @Req() request: Request,
        @Res() response: Response,
    ): Promise<void> {
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/orders/shipping-address/resolve`,
            request,
        );
        response.status(status).json(data);
    }
    ...
    @RequirePermissions(Permission.ORDER_CREATE)
    async proxyShippingAddressWards(
        @Req() request: Request,
        @Res() response: Response,
    ): Promise<void> {
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/orders/shipping-address/ghn-wards`,
            request,
        );
        response.status(status).json(data);
    }
    ...
    async proxyCreateReturn(
        @Param('orderId') orderId: string,
        @Req() request: Request,
        @Res() response: Response,
    ): Promise<void> {
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/orders/${orderId}/returns`,
            request,
        );
        response.status(status).json(data);
    }
    ...
    @RequirePermissions(Permission.RETURN_READ)
    async proxyReturnDetail(
        @Param('returnId') returnId: string,
        @Req() request: Request,
        @Res() response: Response,
    ): Promise<void> {
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/orders/returns/${returnId}`,
            request,
        );
        response.status(status).json(data);
    }
    ...
    async proxyCancelReturn(
        @Param('returnId') returnId: string,
        @Req() request: Request,
        @Res() response: Response,
    ): Promise<void> {
        const { data, status } = await this.proxyService.forward(
            `${this.targetBase}/api/v1/orders/returns/${returnId}/cancellation`,
            request,
        );
        response.status(status).json(data);
    }
